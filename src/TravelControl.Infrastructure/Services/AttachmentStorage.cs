...
public sealed class AttachmentStorage(AppDbContext db, IConfiguration config)
{
    ...
    {
        ["application/pdf"] = ["25504446".Chunk(2).Select(x => Convert.ToByte(new string(x), 16)).ToArray()],
        ["image/png"] = ["89504E470D0A1A0A".Chunk(2).Select(x => Convert.ToByte(new string(x), 16)).ToArray()],
        ["image/jpeg"] = [[0xFF, 0xD8, 0xFF]]
    };
    private readonly string root = Path.GetFullPath(config["Storage:Root"] ?? "/app/storage");
    ...
    public async Task<StoredAttachment> SaveAsync(IFormFile file, DocumentType documentType, Guid userId, string? description,
        Guid? passengerId, Guid? roomId, Guid? flightId, Guid? baggageId, Guid? transferId, CancellationToken ct)
    {
        ...
        if (!allowed.Any(sig => bytes.AsSpan().StartsWith(sig))) throw new InvalidOperationException("El contenido real del archivo no coincide con su tipo MIME.");
        ...
        var entity = new Attachment { DocumentType = documentType, OriginalName = Path.GetFileName(file.FileName), StoredName = storedName,
            ...
            Description = description, PassengerId = passengerId, RoomReservationId = roomId, FlightBookingId = flightId,
            BaggageEntitlementId = baggageId, TransferBookingId = transferId };
