...
const TransfersPage=lazy(()=>import('./pages/OperationsPages').then(m=>({default:m.TransfersPage})))
...
export function App(){return <Routes><Route path="/login" element={<LoginPage/>}/><Route path="/setup" element={<SetupPage/>}/><Route element={<Protected/>}><Route index element={deferred(<DashboardPage/>)}/><Route path="passengers" element={deferred(<PassengersPage/>)}/><Route path="passengers/:id" element={deferred(<PassengerDetailPage/>)}/><Route path="rooms" element={deferred(<RoomsPage/>)}/><Route path="flights" element={deferred(<FlightsPage/>)}/><Route path="baggage" element={deferred(<BaggagePage/>)}/><Route path="transfers" element={deferred(<TransfersPage/>)}/><Route path="pending" element={deferred(<PendingPage/>)}/><Route path="import" element={deferred(<ImportExportPage/>)}/><Route path="audit" element={deferred(<AuditPage/>)}/></Route><Route path="*" element={<Navigate to="/" replace/>}/></Routes>}
