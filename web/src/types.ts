...
export interface Passenger {
  ...
  requirements: RequirementState[]; alerts: string[]; flights:PassengerFlightSummary[]; nextAction?: string; nextActionDueDate?: string; updatedAt: string; version: number
}
...
export interface AirlineSummary {name:string;passengers:number}
export interface DashboardData { kpis: DashboardKpi[]; categories: CategoryProgress[]; overallDistribution: Record<string, number>; operators: OperatorSummary[]; airlines:AirlineSummary[]; priorityActions: PriorityAction[]; recentActivity: RecentActivity[];transfer:TransferStatus;tripReadiness:TripReadiness;roomsConfirmed:number;roomsPending:number;specificPropertiesPending:number }
...
export interface PublicDashboard {
  ...
  categories:CategoryProgress[];operators:{name:string;rooms:number;passengers:number;resolvedRooms:number}[];airlines:AirlineSummary[]
